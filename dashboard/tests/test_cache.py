...
class DashboardCacheTestCase(TestCase):
    ...
    def test_invalidate_dashboard_cache(self):
        ...
        """
        # Cache multiple data entries
        cache_dashboard_data('test1', self.test_data)
        cache_dashboard_data('test2', self.test_data)
        cache_dashboard_data('test1', self.test_data, {'param': 'value'})
        
        # Verify that data is cached
        self.assertIsNotNone(get_cached_dashboard_data('test1'))
        self.assertIsNotNone(get_cached_dashboard_data('test2'))
        self.assertIsNotNone(get_cached_dashboard_data('test1', {'param': 'value'}))
        
        # Invalidate specific prefix
        invalidate_dashboard_cache('test1')
        
        # Verify that only test1 data is invalidated
        self.assertIsNone(get_cached_dashboard_data('test1'))
        self.assertIsNone(get_cached_dashboard_data('test1', {'param': 'value'}))
        self.assertIsNotNone(get_cached_dashboard_data('test2'))
        
        # Cache test1 data again
        cache_dashboard_data('test1', self.test_data)
        ...
        # Verify that all data is invalidated
        self.assertIsNone(get_cached_dashboard_data('test1'))
        self.assertIsNone(get_cached_dashboard_data('test2'))
